...
<div class="sm:flex sm:items-center sm:justify-between block sm:mb-4">
    <ButtonGroup class="w-fit" size="md">
        <InputAddon class="py-2 ease-in duration-200 whitespace-nowrap">จำนวนรายการ</InputAddon>
        <div class="px-4 py-2 min-w-[5.5rem] max-w-[5.5rem] ease-in duration-200 border dark:border-gray-900 text-gray-600 bg-white dark:text-gray-400 dark:bg-gray-800 rounded-r-lg flex items-center justify-end cursor-pointer"><Chevron>{limit}</Chevron></div>
        <Dropdown class="w-fit py-2 rounded-md bg-gray-50 dark:bg-gray-900" transition={slide} bind:open>
    ...
    <div class="sm:my-0 my-4">
        <Input
            ...
            on:keydown={search}
        >
            <svg on:click={() => dispatch("search", { searchText })} slot="left" aria-hidden="true" class="pointer-events-auto cursor-pointer w-6 h-6 text-gray-500 dark:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
