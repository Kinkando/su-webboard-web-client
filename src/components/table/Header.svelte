...
<div class="sm:flex sm:items-center sm:justify-between block sm:mb-4">
    <ButtonGroup class="w-full" size="md">
        ...
        <div class="px-4 py-2 w-16 w-full sm:max-w-[4rem] ease-in duration-200 border dark:border-gray-900 text-gray-600 bg-white dark:text-gray-400 dark:bg-gray-800 rounded-r-lg">{limit}</div>
        <Dropdown class="w-full py-2" transition={slide} bind:open>
            {#each entryList as entry}
                <DropdownItem defaultClass="font-medium py-2 px-4 text-sm hover:bg-gray-100 dark:hover:bg-gray-600 {limit === entry ? 'bg-gray-100 dark:bg-gray-600' : ''}" on:click={() => click(entry)}>{entry}</DropdownItem>
            {/each}
        </Dropdown>
    </ButtonGroup>
