...
<div class="rounded-lg shadow-md w-full h-full p-4 sm:p-6 overflow-hidden bg-white text-black dark:bg-gray-700 dark:text-white ease-in duration-200">
    <div class="flex items-center justify-between mb-4">
        <h1 class="font-bold text-2xl">อาจารย์</h1>
        <Button class="hover:scale-105 ease-in duration-200" color="greenToBlue" gradient>เพิ่มอาจารย์</Button>
    </div>
    <Table bind:limit bind:total {columns} bind:data skeletonLoad multiSelect on:fetch={fetchTeachers} {actions} />
</div>