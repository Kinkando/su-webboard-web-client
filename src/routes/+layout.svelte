...
{#if $page.status === HTTP.StatusOK && isUserSite}
    ...
    <header class="h-16 w-full overflow-hidden flex items-center bg-[var(--primary-color)] px-4 no-select fixed z-20">
        ...
        <nav class="flex items-center gap-x-2 ml-auto">
            <DarkMode id={tooltips[0].id} btnClass="scale-110 text-white hover:bg-white hover:text-[var(--primary-color)] dark:hover:bg-gray-700 dark:hover:text-white rounded-full p-2 transition-all ease-in duration-200" />
            ...
            {#if user?.userType === UserType.TEACHER}
                <a id="{tooltips[2].id}" class="rounded-full text-white hover:bg-white hover:text-[var(--primary-color)] dark:hover:bg-gray-700 dark:hover:text-white p-1 w-10 h-10 relative cursor-pointer transition-all ease-in duration-200" href="/announcement/new">
